...
import { AuthType, debugLogger, OutputFormat } from '@google/coder-cli-core';
...
export async function validateNonInteractiveAuth(
  ...
  try {
    ...
    if (!effectiveAuthType) {
      const message = `Please set an Auth method in your ${USER_SETTINGS_PATH} or specify one of the following environment variables before running: CODER_API_KEY, GOOGLE_GENAI_USE_VERTEXAI, GOOGLE_GENAI_USE_GCA`;
      throw new Error(message);
    }
