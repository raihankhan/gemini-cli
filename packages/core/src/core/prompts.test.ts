...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('CODER_SYSTEM_MD', undefined);
    vi.stubEnv('CODER_WRITE_SYSTEM_MD', undefined);
    mockConfig = {
  ...
  describe('CODER_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should use default prompt when CODER_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('CODER_SYSTEM_MD', value);
        const prompt = getCoreSystemPrompt(mockConfig);
    ...
    it('should throw error if CODER_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('CODER_SYSTEM_MD', customPath);
    ...
    it.each(['true', '1'])(
      'should read from default path when CODER_SYSTEM_MD is "%s"',
      (value) => {
    ...
    it('should read from custom path when CODER_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when CODER_SYSTEM_MD is set', () => {
  ...
  describe('CODER_WRITE_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should not write to file when CODER_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('CODER_WRITE_SYSTEM_MD', value);
    ...
    it.each(['true', '1'])(
      'should write to default path when CODER_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(GEMINI_DIR, 'system.md'));
        vi.stubEnv('CODER_WRITE_SYSTEM_MD', value);
        getCoreSystemPrompt(mockConfig);
    ...
    it('should write to custom path when CODER_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('CODER_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt(mockConfig);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it('should expand tilde in custom path when CODER_WRITE_SYSTEM_MD is set', () => {
      const homeDir = '/Users/test';
      vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
      const customPath = '~/custom/system.md';
      const expectedPath = path.join(homeDir, 'custom/system.md');
    ...
    it('should expand tilde in custom path when CODER_WRITE_SYSTEM_MD is just ~', () => {
      const homeDir = '/Users/test';
      vi.spyOn(os, 'homedir').mockReturnValue(homeDir);
      const customPath = '~';
