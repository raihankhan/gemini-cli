...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    const workspacePath =
      ...
      process.env['CODER_CLI_IDE_WORKSPACE_PATH'];
  ...
  private getPortFromEnv(): string | undefined {
    const port = process.env['CODER_CLI_IDE_SERVER_PORT'];
    if (!port) {
      return undefined;
    }
    return port;
  ...
  private getStdioConfigFromEnv(): StdioConfig | undefined {
    const command = process.env['CODER_CLI_IDE_SERVER_STDIO_COMMAND'];
    if (!command) {
      return undefined;
    }

    const argsStr = process.env['CODER_CLI_IDE_SERVER_STDIO_ARGS'];
    let args: string[] = [];
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          logger.error(
            'CODER_CLI_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
        logger.error('Failed to parse CODER_CLI_IDE_SERVER_STDIO_ARGS:', e);
      }
    }
